...
public class Model {
    ...
    public boolean maxTileExists() {
        for (int i = 0; i < this.size(); i++) {
            for (int j = 0; j < this.size(); j++) {
                if (this.tile(i, j) != null && this.tile(i, j).value() == MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i < this.size(); i++) {
            for (int j = 0; j < this.size(); j++) {
                if (j > 0 && this.tile(i, j).value() == this.tile(i, j - 1).value()) {
                    return true;
                }
                if (i > 0 && this.tile(i, j).value() == this.tile(i - 1, j).value()) {
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int targetY = y;
        for (int i = y + 1; i < this.size(); i++) {
            if (this.tile(x, i) == null) {
                targetY = i;
            } else {
                if (this.tile(x, i).value() == myValue && !this.tile(x, i).wasMerged()) {
                    targetY = i;
                }
                break;
            }
        ...
        if (targetY > y) {
            ...
            if (this.tile(x, targetY).wasMerged()) {
                this.score += this.tile(x, targetY).value();
            }
        }
    }
